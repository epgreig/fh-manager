/** Personal cuts affect the blended valuation, not source forecasts or market ranks. */
function adjustmentEntries_(rows,players) {
  const entries=rows.map(row=>{
    ...
    if(cut!==''&&(typeof cut!=='number'||!Number.isFinite(cut)||cut<0||cut>1))return {player:match.player,cut:0,message:'Enter a cut from 0% to 100%'};
...
function ensureAdjustments_() {
  ...
  s.getRange(1,2).setNote('Enter 25% to retain 75% of the blended fantasy points. Blank or 0% means no cut. Then Draft > Refresh board to update PAR and PAN.');
  ...
  s.getRange(2,2,s.getMaxRows()-1,1).setNumberFormat('0%').setDataValidation(SpreadsheetApp.newDataValidation().requireNumberBetween(0,1).setAllowInvalid(true).build());
