...
function migrateReplacementSettings_() {
  ...
  if(guide) {
    ...
    rows.forEach(r=>{
      ...
      if(r[0]==='Uncertainty')r[1]='sADP = (0.2 × ESPN rank^-2 + 0.4 × ESPN ADP^-2 + 0.4 × Dom PAR rank^-2)^(-1/2), before optional position adjustments. Dom uses his own stats and D/G replacement points with the shared forward baseline. PAN uses sRk among available players, with uncertainty growing with current pick − 1 + sRk.';
...
function setupDraftSheet() {
  ...
  table_('Guide',['Topic','Details'],[
    ...
    ['Uncertainty','sADP uses a power mean (p = -2) with 20% ESPN rank, 40% ESPN ADP, and 40% Dom-only PAR rank, before optional positional adjustments. PAN uses sRk among available players; uncertainty is max(adpSigmaFloor, adpSigmaRate × (current pick − 1 + sRk)).'],
