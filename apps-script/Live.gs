...
function FH_STATE(settings,keepers,log,identities) {
  const c=configFromSettings_(settings);
...
function renderBoard_({c,players,state}) {
  ...
  identities.getRange(1,10).setValue('Dom rank (league PAR)').setNote('Rank by The Athletic projections alone under current Settings scoring minus Dom-only points at the configured replacement ranks. Includes kept/drafted players; dual-eligible forwards use their best eligible PAR.');
  ...
  model.getRange('AB1').setValue('Displayed survival');
  model.getRange(2,28,n,1).setFormulas(result.available.map((p,i)=>{
    const r=i+2;return ['=IF(O'+r+'="","",IF(O'+r+'=1,1,ROUNDDOWN(O'+r+',1)))'];
  }));
  ...
  const rankHeader=leagueConfig_().platform==='Yahoo'?'yahoo':'espn';
  ['F','D','G'].forEach((g,i)=>{
    ...
    const modelColumns={Player:'A',POS:'B',Tm:'C',Age:'T',espn:'Q',ADP:'F',Dom:'X',sADP:'R',sRk:'AA',Surv:'AB',coefV:'Z',PAR:'E'};
    modelColumns[rankHeader]='Q';
    ...
    const widths={Player:145,POS:46,Tm:32,Age:28,espn:38,ADP:38,Dom:45,sADP:42,sRk:42,Surv:34,coefV:45,PAR:40,PAN:40};
    widths[rankHeader]=38;
    headers.filter(h=>h!=='ID').forEach(h=>s.setColumnWidth(col+index[h],widths[h]));
    s.hideColumns(col+index.ID);s.hideColumns(col+index[rankHeader],4);
    ...
    if(index.Surv!==undefined) {
      s.getRange(3,col+index.Surv).setNote('Probability this player is still available at your next pick. Displayed to one truncated decimal; PAN uses full precision.');
      s.getRange(4,col+index.Surv,n,1).setNumberFormat('[=1]0;.0');
    }
    ...
    [rankHeader,'ADP','Dom'].forEach(h=>rankRanges.push(s.getRange(4,col+index[h],n,1)));
...
function showReplacementLevels_(baselines) {
  ...
  s.getRange(1,3).setNote('Replacement points are calculated from full eligible pools on Refresh board, including drafted players and keepers. Yahoo C/LW/RW pools overlap; each player appears once within each eligible pool.');
  s.getDataRange().getValues().forEach((row,i)=>{
    const match=/^replacement(F|C|LW|RW|D|G)$/.exec(String(row[0]));
    if(!match)return;
