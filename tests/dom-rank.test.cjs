...
test('Dom ranks compare source-only PAR, with source-specific D/G replacement points',()=>{
 ...
 const ranks=ctx.domProjectionRanks_(players,rows,{...ctx.c,replacementD:1,replacementG:1});
 assert.equal(ranks.get('f'),1);assert.equal(ranks.get('d'),2);assert.equal(ranks.get('g'),2);
 assert.equal(ranks.has('missing'),false);
 const tie=ctx.domProjectionRanks_(players,rows,{...ctx.c,replacementD:1,replacementG:1,replacementFPoints:200});
 assert.equal(tie.get('f'),3);assert.equal(tie.get('d'),1);assert.equal(tie.get('g'),1);
 const missing=ctx.domProjectionRanks_(players,rows,ctx.c);
 assert.equal(missing.has('d'),false);assert.equal(missing.has('g'),false);
});
