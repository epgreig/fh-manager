...
test('clearing log resets draft state while keepers stay excluded',()=>{
 ...
 assert.equal(ctx.FH_STATE(settings,[['A']],[],ids)[0][0],1);
 assert.equal(ctx.FH_STATE(settings,[['A']],[],ids)[0][2],22);
});
