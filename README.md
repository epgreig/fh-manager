...
Then blend that adjusted ESPN estimate with **Dom rank (league points)**: `final sADP = ESPN estimate^(1 − domRankWeight) × Dom rank^domRankWeight`. `domRankWeight` defaults to 0.25 and updates sADP/PAN live; zero restores the ESPN-only estimate. The 25% weight is a configurable starting assumption, not a fitted estimate of three managers' behavior. Dom rank is calculated from The Athletic rows in Projections alone, scored with current league Settings (including the D bonus), across all players including keepers and drafted players. It uses total fantasy points, not PAR or the workbook's published RK. Ties share rank. See column J of Players; refresh after changing scoring or Dom's stats. Missing Dom ranks fall back to ESPN; missing ESPN inputs use Dom when its weight is positive. Existing positional adjustments apply only to ESPN, not to Dom's league-scored rank. This signal is independent of Dom's weight in the projection blend.
