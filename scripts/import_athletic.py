...
def extract(path, extended=False):
    ...
    for row in rows[1:]:
        ...
        if extended and group != 'G':
            indices.update({'SOG':21,'HIT':27,'PLUS_MINUS':28,'SHG':24})
        stats = {}
        for stat, index in indices.items():
            ...
            stats[stat] = value
        if extended and group != 'G':
            stats['SHA'] = stats['SHP'] - stats['SHG']
            if stats['SHA'] < 0:
                raise ValueError(f'{name}: SHP below SHG')
