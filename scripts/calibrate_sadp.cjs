...
// Usage: node scripts/calibrate_sadp.cjs PRIVATE_HISTORY.json [ESPN.json] [D rank] [G rank] [F rank]
...
const c={...ctx.defaults,replacementD:Number(process.argv[4]||35),replacementG:Number(process.argv[5]||20),replacementF:Number(process.argv[6]||80),espnRankWeight:.2,domRankWeight:.3};
...
console.log(JSON.stringify({snapshot:snapshot.retrievedAt,assumptions:{replacementF:c.replacementF,replacementD:c.replacementD,replacementG:c.replacementG,power:-2,weights:{espnRank:.2,adp:.5,dom:.3},poolSize:pool.length,domRanksMatched:domByEspn.size},targetCounts:targets,neutral:evaluate(1,1),oldCorrections:evaluate(.86,.65,1.235),fittedLinear:best},null,2));
